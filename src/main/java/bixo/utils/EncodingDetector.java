...
public class EncodingDetector {
    ...
    public static final Logger LOGGER = Logger.getLogger(EncodingDetector.class);
    ...
    public void autoDetectClues(FetchedDatum fetchedDatum, String contentType, boolean filter) {
        ...
        if (minConfidence >= 0 && DETECTABLES.contains(fetchedDatum.getContentType())
            ...
            try {
                ...
                LOGGER.debug("Exception from ICU4J (ignoring)", e);
            }
        ...
        addClue(parseCharacterEncoding(contentType), "header");
    }
    ...
    public String guessEncoding(FetchedDatum fetchedDatum, String defaultValue) {
        ...
        for (EncodingClue clue : clues) {
            if (LOGGER.isTraceEnabled()) {
                LOGGER.trace(base + ": charset " + clue);
            }
            String charset = clue.value;
            if (minConfidence >= 0 && clue.confidence >= minConfidence) {
                if (LOGGER.isTraceEnabled()) {
                    LOGGER.trace(base + ": Choosing encoding: " + charset +
        ...
        if (LOGGER.isTraceEnabled()) {
            LOGGER.trace(base + ": Choosing encoding: " + bestClue);
        }
