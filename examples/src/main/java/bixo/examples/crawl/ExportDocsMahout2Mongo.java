    ...
    private int exportDocs(Configuration conf, Path docFiles, DBCollection docs) throws Exception {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                // For reuters this is assumed to be of the form "/-tmp/reut2-000.sgm-0.txt"
                // turn it into "domain/FINDERBOTS_PATH_TO_REUTERS_TXT_FILES/reut2-000.sgm-0.txt"
                // which will be a unique value that can be used as a URL to the text
                // for web pages this should be replaced with the fully qualified path to the page
                // This will be displayed using an iframe element in FinderBots.com
                //Path textFile = new Path(key);
                //key = textFile.getName();
                //textFile = new Path(FINDERBOTS_PATH_TO_REUTERS_TXT_FILES, key);
                //key = textFile.toString();
