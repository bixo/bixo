    ...
    public void readClustersFromMahout() throws Exception {
        try {
            ...
            while (iterator.hasNext()) {// for each clustered doc
                ...
                // TODO: this is pretty heavy weight, there used to be a distance stored with the doc vector
                CosineDistanceMeasure dm = new CosineDistanceMeasure();
                Vector v1 = topics.get(clusterId).getCentroid();
                Vector v2 = clusteredDoc.getVector();
                double dist = dm.distance(topics.get(clusterId).getCentroid(), clusteredDoc.getVector());
...
//        SequenceFileDirIterator<IntWritable, ClusterWritable> iterator = new SequenceFileDirIterator<IntWritable, ClusterWritable>(clusterConf.getClusterFilesPaths(), true, conf);
        SequenceFileDirIterator<IntWritable, ClusterWritable> iterator = (SequenceFileDirIterator<IntWritable, ClusterWritable>) MahoutUtilities.getMultiFileIterator(clusterConf.getClusterFilesPath(), "art-");
        int clusterNumber = 0;
        while (iterator.hasNext()) {// for each clustered doc
