...
abstract class ClusterConfiguration { //abstract class must be overriden for each cluster config
    ...
    public Path getClusterFilesPath() throws IOException {
        return null; //must override in implemented ClusterConfiguration
    }
...
class BixoClusterConfiguration extends ClusterConfiguration {
    ...
    public Path getClusterFilesPath() throws IOException {//points to a dir with another dir that has "final" in it's name
        FileSystem fs = FileSystem.get(conf);
        FileStatus[] dirStats = fs.listStatus(clusterFiles);
        ArrayList<Path> clusterFilesPaths = new ArrayList<Path>();
        for( FileStatus s : dirStats ){
            if(s.getPath().toString().contains("-final")){//clusters-n-final etc.
                return s.getPath();
            } //otherwise its iteration clusters
        }
        IOException e = new IOException("Cannot find directory for the final cluster iteration.");
        throw e;
    }
